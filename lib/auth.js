...
export function verifyToken(token) {
  if (!token || typeof token !== 'string' || token.trim() === '') {
    console.log("DEBUG verifyToken: empty/invalid token");
    return null;
  }
  try {
    const decoded = jwt.verify(token, requireJwtSecret());
    console.log("DEBUG verifyToken: success, id=", decoded.id);
    return decoded;
  } catch (error) {
    console.error("DEBUG verifyToken ERROR:", error.name, error.message);
    return null;
  }
}
